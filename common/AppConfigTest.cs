...
namespace common.test
{
    ...
    public class CasConfigTest
    {
        ...
        public async void GetString()
        {
            ...
            var v0 = AppConfig.GetStringOnce("MY_KEY_vcr1", dflt: "my-default");
            Assert.Equal("my-default", v0);

            // ensure if key is set, that value is used
            var v1 = AppConfig.GetStringOnce("MY_KEY_af3d", dflt: "my-default");
        ...
        public async void GetInt()
        {
            ...
            // ensure if key is not set, the default is taken
            var v0 = AppConfig.GetIntOnce("MY_KEY_irr2", dflt: -1);
            Assert.Equal(-1, v0);

            // ensure if key is set, that value is used
            var v1 = AppConfig.GetIntOnce("MY_KEY_x21d", dflt: -1);
            Assert.Equal(10, v1);
        ...
        public async void GetBool()
        {
            ...
            // ensure if key is not set, the default is taken
            var v0 = AppConfig.GetBoolOnce("MY_KEY_aqws", dflt: true);
            Assert.True(v0);

            // ensure if key is set, that value is used
            var v1 = AppConfig.GetBoolOnce("MY_KEY_zzew", dflt: false);
            Assert.True(v1);
            ...
            var v6 = AppConfig.GetBoolOnce("MY_KEY_zzew", dflt: false);
            Assert.True(v6);
            var v7 = AppConfig.GetBoolOnce("MY_KEY_1i23", dflt: false);
            Assert.True(v7);
            var v8 = AppConfig.GetBoolOnce("MY_KEY_fmfu", dflt: false);
            Assert.True(v8);

            // ensure different FALSE conditions are all false
            var v9 = AppConfig.GetBoolOnce("MY_KEY_i444", dflt: true);
            Assert.False(v9);
            var v10 = AppConfig.GetBoolOnce("MY_KEY_3jej", dflt: true);
            Assert.False(v10);
            var v11 = AppConfig.GetBoolOnce("MY_KEY_49f3", dflt: true);
            Assert.False(v11);
        ...
        public async void GetArray()
        {
            ...
            // ensure if key is not set, the default is taken
            var v0 = AppConfig.GetArrayOnce("MY_KEY_93i3", dflt: new string[] { "yes", "no" });
            Assert.Equal(2, v0.Count());
            Assert.Equal("yes", v0[0]);
            Assert.Equal("no", v0[1]);

            // ensure if key is set, that value is used
            var v1 = AppConfig.GetArrayOnce("MY_KEY_djeu");
        ...
        public async void GetEnum()
        {
            ...
            // ensure if key is not set, the default is taken
            var v0 = AppConfig.GetEnumOnce<SameSiteMode>("MY_KEY_4j4j", dflt: SameSiteMode.None);
            Assert.Equal(SameSiteMode.None, v0);

            // ensure if key is set, that value is used
            var v1 = AppConfig.GetEnumOnce<SameSiteMode>("MY_KEY_3k3k", dflt: SameSiteMode.None);
